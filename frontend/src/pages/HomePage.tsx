...
export const HomePage = () => {
  ...
  const handleNavigateToDashboard = () => {
    // Toujours rediriger vers /dashboard pour les non-connectés
    // Les recruteurs connectés seront redirigés automatiquement par RoleBasedRoute si nécessaire
    ...
    if (userData) {
      try {
        ...
        // En cas d'erreur, permettre l'accès au dashboard public
        console.error('Erreur de parsing user data:', error);
      }
    }
    // Par défaut, rediriger vers le dashboard public
    navigate('/dashboard');
  };
  ...
  const handleSearch = ({ what, where }: { what: string; where: string }) => {
    ...
    // Toujours rediriger vers /dashboard pour les recherches
    // Les recruteurs connectés seront redirigés automatiquement si nécessaire
    const userData = localStorage.getItem('userData');
    ...
    if (userData) {
      try {
        ...
        // Rediriger les recruteurs vers leur dashboard spécifique
        if (user.role === 'RECRUITER') {
          basePath = '/recruteur/dashboard';
        }
      } catch (error) {
        // En cas d'erreur, utiliser le dashboard public
        console.error('Erreur de parsing user data:', error);
      }
    }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white overflow-x-hidden">
      ...
      <main>
        ...
        >
          ...
          <div className="relative z-10 container mx-auto px-4 sm:px-6 py-12 sm:py-16 lg:py-20">
            ...
            <div className="text-center mb-12 space-y-6">
              ...
              <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-50 leading-tight">
              ...
              <div className="flex items-center justify-center gap-2 sm:gap-3 flex-wrap">
                <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-slate-800/50 rounded-lg border border-slate-700/50">
                  <span className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-sky-400 to-blue-500 bg-clip-text text-transparent">
                    {isLoadingStats ? '...' : stats.totalOffers.toLocaleString('fr-FR')}
                  </span>
                  <span className="text-slate-400 text-xs sm:text-sm font-medium">offres actives</span>
                </div>
                <div className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-slate-800/50 rounded-lg border border-slate-700/50">
                  <span className="text-2xl sm:text-3xl font-bold bg-gradient-to-r from-emerald-400 to-teal-500 bg-clip-text text-transparent">
                    {isLoadingStats ? '...' : stats.totalCompanies.toLocaleString('fr-FR')}
                  </span>
                  <span className="text-slate-400 text-xs sm:text-sm font-medium">entreprises</span>
                </div>
              </div>
        ...
        <section className="relative py-12 sm:py-16 lg:py-20 bg-gradient-to-b from-slate-900 to-slate-950">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="text-center mb-8 sm:mb-12">
              <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-100 mb-2 sm:mb-3">
                Recherches populaires
              </h3>
              <p className="text-sm sm:text-base text-slate-400">Explorez les offres par type de contrat</p>
            </div>
        ...
        <section className="py-12 sm:py-16 lg:py-20 bg-slate-950">
          <div className="container mx-auto px-4 sm:px-6">
            <div className="text-center mb-10 sm:mb-16">
              <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-100 mb-3 sm:mb-4">
        ...
        <section className="relative py-12 sm:py-16 lg:py-20 overflow-hidden">
          ...
          <div className="relative container mx-auto px-4 sm:px-6">
            <div className="max-w-4xl mx-auto text-center">
              <div className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 bg-sky-500/10 border border-sky-500/20 rounded-full text-sky-400 text-xs sm:text-sm font-medium mb-4 sm:mb-6">
                <span>⭐</span>
                <span className="hidden sm:inline">Démarquez-vous auprès des recruteurs</span>
                <span className="sm:hidden">Démarquez-vous</span>
              </div>

              <h3 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-slate-100 mb-4 sm:mb-6 px-4">
                Déposez votre CV et soyez visible par les entreprises
              </h3>

              <p className="text-slate-300 text-base sm:text-lg mb-8 sm:mb-10 max-w-2xl mx-auto px-4">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
                <button
                  ...
                  className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white text-sm sm:text-base font-bold rounded-xl transition-all duration-300 transform hover:scale-105 shadow-xl shadow-sky-500/30 hover:shadow-sky-500/50 focus:outline-none focus:ring-2 focus:ring-sky-500 focus:ring-offset-2 focus:ring-offset-slate-950"
                >
                ...
                <button
                  ...
                  className="w-full sm:w-auto px-6 sm:px-8 py-3 sm:py-4 bg-slate-800 hover:bg-slate-700 text-slate-200 text-sm sm:text-base font-semibold rounded-xl border border-slate-700 hover:border-slate-600 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-slate-500"
                >
                  Explorer les offres
                </button>
              </div>

              <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-8 text-xs sm:text-sm text-slate-400">
      ...
      {/* ==================== FOOTER ==================== */}
      <footer className="relative border-t border-slate-800/50 bg-slate-950 py-8 sm:py-12">
        <div className="container mx-auto px-4 sm:px-6">
          ...
          {/* Copyright */}
          <div className="pt-6 sm:pt-8 border-t border-slate-800/50 text-center">
